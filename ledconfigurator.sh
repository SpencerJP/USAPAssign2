	...
	while [ $QUIT_TRIGGER_MENU -eq 0 ] ; do 
		...
		do
			if [[ $option = "${CHOICE}" ]] ; then
				...
				continue 2 #continue main while loop, not this loop. 
			fi
			((option++))
		done
		quitnum=${#event_array[@]}
...
led_menu(){
	...
	while [ $QUIT_LED_MENU -eq 0 ] ; do 
		...
		echo "Invalid input." #their input must be invalid if it missed the continue statements.
	done
}
...
while [ $QUIT -eq 0 ] ; do
	...
	for lednumber in "${!LEDLIST[@]}"; do
		if [[ $lednumber = "${CHOICE}" ]] ; then
			...
			CHOICE=-1
			continue 2 # force continue while loop to avoid error message at bottom of loop
		fi
	done
